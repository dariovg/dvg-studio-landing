...
export function wantsBooking(text) {
  ...
  if (/\b(quedar|quedamos|nos vemos|vernos|concertamos|quedaria)\b/.test(t)) {
    return true;
  }

  if (/\b(podemos|quiero|podria|te viene|nos podemos|cuando|gustaria)\b/.test(t) && /\b(ver|hablar|llamar|quedar)\b/.test(t)) {
    return true;
  }
  ...
  if (/\b(cita|reunion|videollamada)\b.*\b(reservar|agendar|concertar)\b/.test(t)) {
    return true;
  }
