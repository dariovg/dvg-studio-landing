...
function futureBusinessSlots(date) {
  return listBusinessSlots().filter((t) => !isInPast(date, t));
}

function calendarDegradedResult({ date, time, error }) {
  const slots = futureBusinessSlots(date);
  ...
  if (normalizedTime && !slots.includes(normalizedTime)) {
    return {
      ok: false,
      reason: "calendario_error",
      available: false,
      time: normalizedTime,
      slots,
      alternatives: slots.slice(0, 5),
  ...
  return {
    ok: false,
    reason: "calendario_error",
    available: normalizedTime ? slots.includes(normalizedTime) : slots.length > 0,
...
export async function getAvailability({ date, time, maxAlternatives = 5 }) {
  ...
  try {
    ...
    busy = await fetchBusyForDay(client, calendars, date);
  } catch (err) {
    console.error("Calendar availability:", err.message);
    return calendarDegradedResult({
      date,
      time: normalizedTime,
      error: err.message,
    });
...
export async function assertSlotAvailable(date, time) {
  ...
  if (
    !result.ok &&
    (result.reason === "sin_calendario" || result.reason === "calendario_error")
  ) {
    return {
      available: result.available !== false,
      alternatives: result.alternatives || result.slots || [],
      degraded: true,
    };
  }
  if (!result.ok) {
