...
import { ANNUAL_DISCOUNT_PERCENT, FIRST_MONTH_IA_DISCOUNT_PERCENT, planById } from "./pricing-config.js";
...
export function buildPricingGuideEmail(lead) {
  ...
  const text = [
    ...
    `Promo confianza mutua: −${FIRST_MONTH_IA_DISCOUNT_PERCENT}% mantenimiento mes 1 (packs IA). Facturación anual: pago único 12 meses — 11 meses a −${ANNUAL_DISCOUNT_PERCENT}%, mes 1 con −${FIRST_MONTH_IA_DISCOUNT_PERCENT}% adicional sobre esa tarifa. No aplica a implementación, web ni consultoría. Mín. 3 meses.`,
    "",
    PLAN_INTRO,
    "",
    howWeWorkInfographicText(),
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:580px;background:#fff;border-radius:20px;overflow:hidden;box-shadow:0 12px 40px rgba(10,14,39,.1);">
        ...
        <tr><td class="email-inner-pad" style="padding:28px 24px;color:#1a2332;font-size:15px;line-height:1.6;">
          ...
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:0 0 20px;border-radius:14px;overflow:hidden;border:1px solid #fde8d4;">
            <tr><td style="padding:16px 18px;background:linear-gradient(135deg,#fff8f3 0%,#fff 100%);">
              <p style="margin:0 0 6px;font-size:14px;font-weight:800;color:#0A0E27;">🤝 Promo confianza mutua — nuevos clientes IA</p>
              <p style="margin:0;font-size:13px;line-height:1.55;color:#444;">
                <strong>−${FIRST_MONTH_IA_DISCOUNT_PERCENT}%</strong> en el mantenimiento del <strong>primer mes</strong> de cualquier pack IA (Starter, Pro, Enterprise).
                Solo el fee mensual de agentes — la implementación, web y consultoría van aparte. Compromiso mínimo 3 meses.
          ...
          ${pricingPlansInfographicHtml()}
          ${agentEducationInfographicHtml()}
          <p style="font-size:12px;color:#888;margin:0 0 20px;text-align:center;line-height:1.5;">${TIMELINES}<br>Facturación anual: pago único 12 meses con −${ANNUAL_DISCOUNT_PERCENT}% y promo mes 1 IA.</p>
