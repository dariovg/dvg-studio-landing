...
import { leadsDatabaseConfigured } from "../leads-store.js";
import { crmConfigured } from "../crm-ingest.js";
...
export default async function handler(req, res) {
  ...
  const checks = {
    ...
    bedrockModel: !!(process.env.BEDROCK_MODEL_ID || "amazon.nova-micro-v1:0"),
    chatBedrockMode: process.env.CHAT_BEDROCK_MODE || "fallback",
    sesFromEmail: !!from,
    ...
    leadsDatabase: leadsDatabaseConfigured(),
    crmIngest: crmConfigured(),
  };
  ...
  } else if (checks.gmailSend) {
    hints.push(
      "La guía por email usa Gmail (gmail.send). Si falla, regenera GOOGLE_REFRESH_TOKEN con scripts/get-google-refresh-token.mjs."
    );
  } else if (checks.awsCredentials) {
    hints.push(
      "SES en sandbox solo envía a correos verificados. Pide Production access en AWS o añade Google OAuth con gmail.send."
    );
  }
  hints.push(
    `Chat IGNITE: modo ${checks.chatBedrockMode}. FAQ + conocimiento local primero; Bedrock solo como último recurso (modelo recomendado: nova-micro).`
  );
  if (checks.chatBedrockMode === "always") {
    hints.push("CHAT_BEDROCK_MODE=always aumenta coste AWS. Usa fallback u off si las consultas son básicas.");
  }
  ...
  if (!checks.crmIngest) {
    hints.push(
      "Configura CRM_API_URL=https://crm.dvgsstudio.com y CRM_INGEST_SECRET (misma clave que en dvg-crm) para sincronizar leads y citas al CRM."
    );
  }
