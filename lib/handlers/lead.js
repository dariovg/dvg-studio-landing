...
export default async function handler(req, res) {
  ...
  if (!emailConfigured()) {
    const dbResult = await saveLead(lead, { emailed: false });
    syncLeadToCrm(lead, { emailed: false }).catch(() => {});
    if (dbResult.ok) {
      return res.status(200).json({
        ok: true,
        emailed: false,
        stored: true,
        message: `Gracias, ${lead.name.split(" ")[0]}. Hemos recibido tus datos. Te contactamos pronto en ${lead.email}.`,
      });
    }
  ...
  try {
    ...
    if (!sent.ok) {
      ...
      if (dbResult.ok) {
        return res.status(200).json({
          ...
          stored: true,
          received: true,
          message: `Gracias, ${lead.name.split(" ")[0]}. Hemos guardado tu solicitud y te contactamos en ${lead.email}.`,
        });
      }
    ...
    const dbResult = await saveLead(lead, { emailed: true });
    if (!dbResult.ok && leadsDatabaseConfigured()) {
      console.error("Lead DB save failed:", dbResult.reason);
    }

    syncLeadToCrm(lead, { emailed: true }).catch(() => {});
