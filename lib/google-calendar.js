...
export async function deleteBookingEvent(eventId) {
  if (!eventId || !calendarConfigured()) {
    return { ok: false, error: "Google Calendar no configurado o sin ID de evento" };
  }
  try {
    const calendar = getCalendarClient();
    ...
    if (err.code === 404 || err.status === 404) return { ok: true, provider: "google", missing: true };
    return { ok: false, error: err.message || "Error al cancelar en Google Calendar" };
  }
}

export async function updateBookingEvent(eventId, booking, { generateMeet = true } = {}) {
  if (!eventId || !calendarConfigured()) {
    return { ok: false, error: "Google Calendar no configurado o sin ID de evento" };
  }
  const parsed = parseDateTime(booking.date, booking.time);
  ...
  try {
    ...
    const meetUrl = extractMeetLink(existing.data);
    const requestBody = {
      summary: `DVG Studio — ${booking.name}`,
      description: buildDescription(booking, meetUrl),
      start: { dateTime: parsed.startStr, timeZone: tz },
      end: { dateTime: parsed.endStr, timeZone: tz },
    ...
    return {
      ...
      provider: "google",
      eventId: event.data.id,
      meetUrl: extractMeetLink(event.data) || meetUrl || undefined,
    };
  } catch (err) {
    if (err.code === 404 || err.status === 404) {
      return createBookingEvent(booking, { generateMeet });
    }
    return { ok: false, error: err.message || "Error al actualizar Google Calendar" };
  }
}
