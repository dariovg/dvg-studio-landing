...
export function pricingPlansInfographicHtml() {
  const rows = PLANS.map((plan, i) => {
    ...
    const badge = plan.badge
      ? `<span style="display:inline-block;background:${BRAND.orange};color:#fff;font-size:10px;font-weight:700;padding:3px 8px;border-radius:999px;margin-left:6px;">${plan.badge}</span>`
      : "";
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td style="vertical-align:top;width:58%;">
              <p style="margin:0 0 4px;font-size:15px;font-weight:800;color:${isPro ? BRAND.orange : BRAND.navy};">${plan.name}${badge}</p>
            ...
            <td style="vertical-align:top;text-align:right;width:42%;">
              ...
              <p style="margin:8px 0 0;font-size:11px;color:${BRAND.blue};font-weight:700;background:#eef6fc;padding:6px 8px;border-radius:8px;display:inline-block;">
  ...
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:0 0 20px;border-radius:16px;overflow:hidden;border:1px solid #e8ecf2;">
    <tr><td style="padding:14px 16px;background:linear-gradient(135deg,${BRAND.navy} 0%,${BRAND.navyMid} 100%);text-align:center;">
      ...
      <p style="margin:0;font-size:13px;color:#ccc;">Mín. 3 meses · implementación aparte</p>
    </td></tr>
    ${rows}
    <tr><td style="padding:10px 16px;background:${BRAND.light};text-align:center;font-size:11px;color:${BRAND.gray};">
...
export function surveyBlockHtml({ beforeMeeting = false } = {}) {
  ...
  const cta = url
    ? `<a href="${url}" style="display:inline-block;background:${BRAND.blue};color:#fff;text-decoration:none;padding:12px 22px;border-radius:999px;font-weight:700;font-size:13px;margin-top:12px;">Rellenar encuesta (3 min)</a>`
