/** Hora España en servidor UTC (Vercel). Ajusta con BOOKING_TZ_OFFSET_MINUTES si hace falta. */
export function tzOffsetMinutes(when = new Date()) {
  ...
  return m >= 2 && m <= 9 ? 120 : 60;
}

export function wallClockToDate(year, month, day, hour, minute) {
  const off = tzOffsetMinutes(new Date(Date.UTC(year, month - 1, day, 12, 0)));
